...
Page({
    ...
    if (reg.test(that.data.perInfo.idCardNumber) === false) {
      ...
      for (var i = 0; i < tempOwner.length; i++) {
        ...
        if (i == tempOwner.length - 1) {
          wx.navigateTo({
            url: '../fillPerDogInfo/fillPerDogInfo',
          })
        }
      }
    }
  ...
  onLoad: function (options) {
    ...
    if (info != null) {
      that.setData({
        'perInfo.personName': info.personName,
        'perInfo.address': info.address,
        'perInfo.idCardNumber': info.idCardNumber,
        'perInfo.personPhoneNumber': info.phoneNumber,
        disable: true
      })
      if (info.personSex == 'F') {
        that.setData({
          F: true,
          'perInfo.personSex': info.personSex,
        })
      }
      if (info.personSex == 'M') {
        that.setData({
      ...
      wx.request({
        ...
        success(res) {
          ...
          that.setData({
            'perInfo.personName': res.data.data.name,
            'perInfo.address': res.data.data.address,
            'perInfo.idCardNumber': res.data.data.idCardNumber,
            'perInfo.personPhoneNumber': res.data.data.phoneNumber,
            disable: true
          })
          if (res.data.data.sex == 'F') {
            that.setData({
              F: true,
              'perInfo.personSex': res.data.data.sex,
            })
          }
          if (res.data.data.sex == 'M') {
            that.setData({
              M: true,
              'perInfo.personSex': res.data.data.sex,
            })
