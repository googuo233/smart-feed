...
Page({
  ...
  btn() {
    ...
    if (reg.test(that.data.perInfo.chargeIdCardNumber) === false) {
      ...
      for (var i = 0; i < tempOwner.length; i++) {
        ...
        if (i == tempOwner.length - 1) {
          wx.navigateTo({
            url: '../newVaccinesDogUnit/newVaccinesDogUnit',
          })
        }
      }
    }
  ...
  onLoad: function (options) {
    ...
    if (info != null) {
      that.setData({
        'perInfo.unitName': info.date.unitName,
        'perInfo.chargeName': info.date.chargeName,
        'perInfo.chargeSex': info.date.chargeSex,
        'perInfo.address': info.date.address,
        'perInfo.phoneNumber': info.date.phoneNumber,
        'perInfo.chargeIdCardNumber': info.date.chargeIdCardNumber,
        disable: false
      })
