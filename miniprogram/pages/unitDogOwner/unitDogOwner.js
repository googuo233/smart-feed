...
Page({
  ...
  onLoad: function (options) {
    ...
    var info = page[1].__data__.personInfo
    console.log(info)
    //自动填写
    if(info!=null){
    that.setData({
      'param.unitName': info.unitName,
      'param.address': info.address,
      'param.idCardNumber': info.chargeIdCardNumber,
      'param.personInCharge': info.chargeName,
    ...
    if (info.chargeSex == 'F') {
      that.setData({
        f: true,
        'param.chargeSex': info.chargeSex,
      })
    }
    if (info.chargeSex == 'M') {
      that.setData({
